...
class Distribute(commands.Cog):
    ...
    @app_commands.command(name="분배", description="보스 수익금을 분배합니다. (올려야 할 잡템 가격 포함)")
    ...
    async def split_meso(self, interaction: discord.Interaction, amount: int, people: int, percents: str = None):
        ...
        # 총 금액에서 3% 경매장 수수료 차감한 실제 분배 풀[cite: 2]
        ...
        if not percents:
            ...
            result_text = f"💰 **총 판매 금액:** {amount:,} 메소\n"
            result_text += f"💸 **실제 정산 대상 (3% 차감):** {actual_amount:,} 메소\n\n"
